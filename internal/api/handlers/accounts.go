...
func NewAccountHandler(db *sql.DB, logger utils.Logger, config *utils.Config) *AccountHandler {
...
func (h *AccountHandler) CreateAccount(c *gin.Context) {
	...
	if digits < 6 || digits > 8 {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Digits must be between 6 and 8"})
		return
	}

	if period < 15 || period > 300 {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Period must be between 15 and 300 seconds"})
		return
	}
