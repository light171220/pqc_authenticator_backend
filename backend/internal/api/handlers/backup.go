...
func (h *BackupHandler) VerifyBackup(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		h.logger.Warn("Invalid verify backup request", "error", err)
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid request format"})
		return
	}
	...
	decryptedData, err := crypto.DecryptDataWithPassword(req.BackupData, req.Password)
	if err != nil {
		h.logger.Warn("Failed to decrypt backup data for verification", "user_id", userID, "error", err)
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid backup data or password"})
		return
	}
	...
	var backupData BackupData
	if err := json.Unmarshal(decryptedData, &backupData); err != nil {
		h.logger.Error("Failed to unmarshal backup data for verification", "error", err)
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid backup format"})
		return
	}
	...
	if err := storage.LogAuditEvent(h.db, userID, "", "backup_verified", map[string]interface{}{
		"backup_version": backupData.Version,
		"backup_date":    backupData.CreatedAt.Unix(),
		"devices_count":  len(backupData.Devices),
		"accounts_count": len(backupData.Accounts),
	...
	h.logger.Info("Backup verified", "user_id", userID, "backup_date", backupData.CreatedAt)
